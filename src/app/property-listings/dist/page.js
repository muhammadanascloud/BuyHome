...
function PropertyListings() {
    ...
    var highlightText = function (typedText) {
        var words = typedText.split(" ");
        return (React.createElement("span", null, words.map(function (word, index) {
            if (word === "Home" || word === "Search") {
                return (React.createElement("span", { key: index, className: "inline-block font-bold bg-gradient-to-r from-accent to-highlight text-white px-2 py-1 mx-1" }, word));
            }
            return (React.createElement("span", { key: index, className: "mx-1" }, word));
        })));
    };
    ...
    var renderPagination = function () {
        ...
        var _loop_1 = function (i) {
            pageNumbers.push(React.createElement("button", { key: i, onClick: function () { return paginate(i); }, className: "px-3 py-2 rounded-lg " + (currentPage === i
                    ? "bg-accent text-heading"
                    : "bg-background text-text") + " hover:bg-highlight transition-colors" }, i));
        };
        ...
        if (endPage < totalPages) {
            pageNumbers.push(React.createElement("button", { key: totalPages, onClick: function () { return paginate(totalPages); }, className: "px-3 py-2 rounded-lg " + (currentPage === totalPages
                    ? "bg-accent text-heading"
                    : "bg-background text-text") + " hover:bg-highlight transition-colors" }, totalPages));
        }
        return pageNumbers;
    };
                ...
                React.createElement("form", { className: "flex flex-col md:flex-row flex-wrap items-center gap-4 mb-6", onSubmit: handleSearch },
                    ...
                    React.createElement("div", { className: "flex flex-col md:flex-row gap-4 w-full md:w-auto" },
                        React.createElement("div", { className: "relative w-full md:w-auto" },
                            React.createElement("select", { value: selectedListingType, onChange: function (e) {
                                    return setSelectedListingType(e.target.value);
                        ...
                        React.createElement("div", { className: "relative w-full md:w-auto" },
                            React.createElement("select", { value: selectedPropertyType, onChange: function (e) {
                                    return setSelectedPropertyType(e.target.value);
                                }, className: "w-full px-4 py-2 border rounded-none text-black outline-none bg-white" },
